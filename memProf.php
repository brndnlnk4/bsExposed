	...
	<div class='col-lg-2'>&nbsp;</div>
	 <div class='col-lg-8' id='picSection'>
	  <p align='center' class='img-responsive img-rounded' id='dfault_pic'>
	   ...
	   <?// default pic-preview \\?>
	   <img src='css/pix/Brandon.jpg' width='130px' alt='Pics' /> 
		...
		$file_size = $file['size'];			
		  ...
		  if(in_array($file_ext, $allowed_ext)){ 					
			if($file_size <= 1000000){								
			   ...
			   $file_destination = $_SERVER['DOCUMENT_ROOT']."/bs_exposed/upl/".$file_name_new;
...
$chk="SELECT `members`.`mem_username`
	...
	|| isset($_POST['short_info'])){
		 ...
		 $_POST['long_info'] = ucfirst(mysqli_real_escape_string($dbCon,$_POST['long_info']));
			///chk email \\
			$email_ext = strtolower(end(explode('.', $_POST['mem_email'])));	
			$exts = array('com','org','net');
...
if(!empty(($_POST['social_google']) 
	 ...
	 || !empty(($_POST['social_youtube']) 
			&& strlen($_POST['social_youtube']) > '0') 
	 || !empty(($_POST['social_linkedin']) 
			&& strlen($_POST['social_linkedin']) > '0')){
	 $_POST['social_google'] = mysqli_real_escape_string($dbCon,$_POST['social_google']);  
	 $_POST['social_facebook'] = mysqli_real_escape_string($dbCon,$_POST['social_facebook']);  
	 $_POST['social_tumblr'] = mysqli_real_escape_string($dbCon,$_POST['social_tumblr']);  
	 $_POST['social_twitter'] = mysqli_real_escape_string($dbCon,$_POST['social_twitter']);  
	 ...
	 $_POST['social_linkedin'] = mysqli_real_escape_string($dbCon,$_POST['social_linkedin']);  
		  $soc = "UPDATE `members`					
				  SET `mem_google` = '{$_POST['social_google']}',
...
<?// MEMBER PROFILE EDIT \\?><?// MEMBER PROFILE EDIT \\?>
   <div id='pic_upl_form'>
	<form enctype="multipart/form-data" action="" target='_self' method="POST" >	  
	  ...
	  <input name="uploadedfile" type="file" onMouseUp='captionInputEnable()' required />
	...
	<div class='center-block' id='<?=rem_picGal()?>'> 
	  ...
	  <input type='text' size='30' maxlength='30' name="caption" value='' class='form-control input-sm ' placeholder='Photo details' id='imgCaptionInput' title='Choose a Photo then Enter details here' disabled />  
		<div class='pull-left' style='border:10px solid transparent;border-radius:8px;box-shadow:0px 3px 5px #111;'>
		  <input type="submit" name="" value="Upload" type='button' class='btn btn-default pull-left' style='color:#999;font-weight:bold;' ></input>
...
<!--- MODAL FOR PICTURE INFO EDITING ------>
 <div id='capEditModal' class='modal' >
  <div class='modal-content'>
   <button type='button' class='close' id='spanClose' style='border:4px outset #888;background:#ddd;'>Close</button>
...
<div class='modal' id='modal_fornoobs' >
  <div class='modal-header' style='display:block;width:40%;margin:5% auto;background-color:rgba(0,0,0,0.75);border-radius:10px;border:3px solid #777;'>
	 ...
	 <p align='center' valign='center' id='noob_rules'  >
	  <h3>
		...
		<ul class='list-group' style='width:95%;padding:10px 20px 10px 20px;color:#222;letter-spacing:-.74px;font-size:18px;line-height:2.1;'>
			...
			<li class='list-group-item' style='margin-bottom:3px;text-align:center;'>
			 You can also Updater profile and set to public or private mode.
 			</li>		
...
?>		   
		...
		<?/// SHOWS MESSAGES FROM OTHER USERS \\\?>	
		   ...
		   <?// members messages \\?>
			<a href='<?="?YoMsg=".md5('read')."#trgt3"?>' target='_self' type='button' class='btn btn-default btn-sm' data-toggle='collapse' data-target='#showMsgBox' style='width:35%;padding:0px 2px;margin:-5px auto;border-radius:10px 10px;background-color:rgba(0,0,0,0.72);border:none;'>
 			  <b>
		...
		<div class='<?=showElementIfRequest("collapse in", "collapse", "YoMsg")?>'  id='showMsgBox'>
			   ...
			   <?php
			    $qry = "SELECT `id`,
								...
								`message_date`
						FROM `member_messages`
						WHERE `message_receiver` LIKE '{$_SESSION['username']}'
	...
	?>
	 <tr>
	   <form class='form form-inline' action='<?=htmlspecialchars($_SERVER['PHP_SELF'])?>#navElement' method='POST' > 
		...
		<td>
			<input class='form-control' type='text' size='30' name='mem_game' placeholder='Better Be 4 PC' value=''  style='width: 100%;'   /> 
		   </td>
	   </tr>
		<tr>
		 <td  style='color:#222;'>		
		...
		<td  >			
			<input class='form-control' type='text' size='30' name='mem_music' placeholder='Music you like'  style='width: 100%;'   /> 
		 </td>
	   </tr>
		<tr>
		 <td  style='color:#222;'>			 
		...
		<td  >			
			<input class='form-control' type='text'  cols='2' size='50' name='mem_sport'  placeholder='Sports or Teams you like' style='width: 100%;'   /> 
		 </td>
	   </tr>
		<tr>
		 <td  style='color:#222;'>			 
		...
		<td >			
			<input class='form-control' type='phone' rows='4' cols='2' size=' ' name='mem_phone'  placeholder='*** *** ****' style='width: 100%;'   /> 
		 </td>
	   </tr>
		<tr>
		 <td  style='color:#222;'>			 
		...
		<td >			
			<input class='form-control' type='email' rows='4' cols='2' size='50' name='mem_email'  placeholder='Email@you.com' style='width: 100%;'   /> 
		 </td>
		<td  style='color:#222;'>		     
	   </tr>
		<tr>
		 <td  style='color:#222;'>	
		   ...
		   <td >			
			<input class='form-control' maxlength='50' type='text' name='short_info' value='' placeholder='Are you Outgoing, Narcistic, Troll' style='width: 90%;' size='50'   />
		  </td>
		 </tr>
		 <tr>
